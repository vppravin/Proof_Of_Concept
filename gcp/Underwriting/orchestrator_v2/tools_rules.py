...
def apply_uw_rules(gcs_folder_uri: str, client_history_json: str = "{}") -> dict:
    """Applies underwriting business rules to a submission's extracted fields.

    Reads extracted fields directly from GCS (_extracted_fields.json) and fetches
    rules automatically. No need to pass large JSON strings.
    ...
    """
    logger.info(f"Tool: apply_uw_rules for {gcs_folder_uri}")

    # Read extracted fields from GCS
    from google.cloud import storage as _gcs
    _client = _gcs.Client()
    bucket_name = "underwriting-workbench"
    blob_path = gcs_folder_uri.replace(f"gs://{bucket_name}/", "").strip("/") + "/_extracted_fields.json"
    try:
        fields = json.loads(_client.bucket(bucket_name).blob(blob_path).download_as_text())
    except Exception as e:
        return {"error": f"Cannot read extracted fields from GCS: {e}"}
    ...
    try:
        ...
        blob_path = gcs_folder_uri.replace("gs://underwriting-workbench/", "").strip("/") + "/_uw_result.json"
        _client.bucket("underwriting-workbench").blob(blob_path).upload_from_string(
