...
echo "🚀 Starting Lead Agent Stream Deployment..."
echo ""
...
IMAGE_NAME="lead-agent-stream"
IMAGE_TAG="v31"
FULL_IMAGE="${REGISTRY_SERVER}/${IMAGE_NAME}:${IMAGE_TAG}"
CONTAINER_APP_NAME="lead-agent-stream"
...
if az containerapp show --name ${CONTAINER_APP_NAME} --resource-group ${RESOURCE_GROUP} &>/dev/null; then
    ...
    az containerapp create \
        ...
        --env-vars \
            ...
            MCP_SERVER_URL=https://lead-agent-mcp.livelybay-96634f40.eastus2.azurecontainerapps.io/mcp

    echo "⚠️  IMPORTANT: Manually add AZURE_OPENAI_API_KEY via:"
    echo "  az containerapp update --name ${CONTAINER_APP_NAME} --resource-group ${RESOURCE_GROUP} --set-env-vars AZURE_OPENAI_API_KEY=<your-key>"
fi
...
echo "✅ Lead Agent Stream deployed successfully!"
echo ""
...
echo "  curl -X POST https://${APP_URL}/query -H 'Content-Type: application/json' -d '{\"query\": \"Show me top leads\"}'"
