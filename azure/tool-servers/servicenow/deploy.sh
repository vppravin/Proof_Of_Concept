...
IMAGE_NAME="servicenow-tool-server"
IMAGE_TAG="v1"
FULL_IMAGE="${REGISTRY_SERVER}/${IMAGE_NAME}:${IMAGE_TAG}"
CONTAINER_APP_NAME="servicenow-tool-server"
...
if az containerapp show --name ${CONTAINER_APP_NAME} --resource-group ${RESOURCE_GROUP} &>/dev/null; then
    ...
    az containerapp create \
        ...
        --max-replicas 2 \
        --env-vars \
            SN_INSTANCE="virtusacorporationdemo18" \
            SN_USERNAME="midhunav" \
            SN_PASSWORD="2178@Ranji" \
