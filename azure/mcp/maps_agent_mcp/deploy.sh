...
# Maps Agent MCP Deployment Script

set -e

echo "🚀 Starting Maps Agent MCP Deployment..."
echo ""
...
IMAGE_NAME="maps-agent-mcp"
IMAGE_TAG="v1"
FULL_IMAGE="${REGISTRY_SERVER}/${IMAGE_NAME}:${IMAGE_TAG}"
CONTAINER_APP_NAME="maps-agent-mcp"
...
if az containerapp show --name ${CONTAINER_APP_NAME} --resource-group ${RESOURCE_GROUP} &>/dev/null; then
    ...
    az containerapp create \
        ...
        --max-replicas 3 \
        --env-vars \
            GOOGLE_MAPS_API_KEY=AIzaSyD6ccqwLYpnmL4emRQduQNiskEBB_0YJ-8
fi
...
echo "✅ Maps Agent MCP deployed successfully!"
echo ""
